...
def compute_confusion_matrix(df: pd.DataFrame, positive_class: int = 1):
    ...
    """
    tp = ((df['actual_class_id'] == positive_class) & (df['predicted_class'] == positive_class)).sum()
    fp = ((df['actual_class_id'] != positive_class) & (df['predicted_class'] == positive_class)).sum()
    tn = ((df['actual_class_id'] != positive_class) & (df['predicted_class'] != positive_class)).sum()
    fn = ((df['actual_class_id'] == positive_class) & (df['predicted_class'] != positive_class)).sum()
